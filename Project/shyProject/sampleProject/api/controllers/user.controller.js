...
function create(req, res) {
    ...
    if (!_.isEmpty(data)) {
        ...
        checkUserExists(condition, false).then(exists => {
            if (exists) {
                res.status(400).send({ "message": "User already Exists with the Given Email" });
            } else {
...
function activate(req, res) {
    ...
    var id = req.swagger.params.id.value || "";
    if (_.isEmpty(id)) {
        res.status(400).send({ message: "Send Proper user Id" });
    } else {
        id = Mongoose.Types.ObjectId(id);
        ...
        updateUser(condition, setObject).then(() => {
            ...
            res.status(200).send({ message: "Error Occured While activating the User " + err.message });
        });
    }
}
